...
детерминированному шаблону.
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.markdown("**Карта модельного риска**")
        st.dataframe(pd.DataFrame([
            {"риск": "Устаревший каталог ставок", "мера": "SLA свежести + детектор дрейфа + REFRESH"},
            {"риск": "Ошибка в расчёте APY", "мера": "юнит-тесты формул, ядро — источник истины"},
            {"риск": "Галлюцинация LLM", "мера": "groundedness-проверка + откат к шаблону"},
            {"риск": "Prompt injection", "мера": "guardrail + текст пользователя как данные"},
            {"риск": "Мисселинг (завышенные ставки)", "мера": "анти-мисселинг guardrail"},
            {"риск": "Сумма вне страховки АСВ", "мера": "риск-флаг 1,4 млн ₽"},
